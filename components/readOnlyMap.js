...
export default function ReadOnlyMap({ 
  ...
  showTankerTracking = true,
  isLiveTracking = false,
  lat = null, // Add direct latitude prop
  lng = null, // Add direct longitude prop
  destinationLat = null, // Destination latitude
  destinationLng = null, // Destination longitude
  zoomLevel = 15, // Default zoom level
  showDestination = true // Whether to show destination marker
}) {
  const webviewRef = useRef(null);
  const [isWebViewReady, setIsWebViewReady] = useState(false);
  const tankerPathRef = useRef([]);
  ...
  // Handle tanker location updates
  useEffect(() => {
    if (isWebViewReady && showTankerTracking) {
      console.log("📍 Sending tracking status to WebView:", isLiveTracking);
  ...
  }, [isLiveTracking, tankerLocation, isWebViewReady, showTankerTracking]);

  // Update tanker location on map
  useEffect(() => {
    if (tankerLocation && isWebViewReady && showTankerTracking && isLiveTracking) {
      console.log("📍 Updating tanker location on map:", tankerLocation);
  ...
  // Handle direct lat/lng coordinates (PRIORITY 1)
  useEffect(() => {
    if (isWebViewReady && lat !== null && lng !== null) {
      console.log("📍 Setting map to coordinates:", lat, lng);
      ...
      if (destinationLat !== null && destinationLng !== null) {
        ...
        // Keep existing address-based destination logic
        destinationCoordsRef.current = center;
      }
  ...
  }, [lat, lng, destinationLat, destinationLng, zoomLevel, showDestination, isWebViewReady]);

  // Handle address geocoding (FALLBACK - only if lat/lng not provided)
  useEffect(() => {
    if (address && isWebViewReady && (lat === null || lng === null)) {
      console.log("📍 Geocoding address (fallback):", address);
      ...
      if (coordMatch) {
        ...
        // Fallback to default location if no lat/lng and address not parseable
        const defaultLocation = { lat: 33.6844, lng: 73.0479 };
        destinationCoordsRef.current = defaultLocation;
        postToWebView({ 
          type: "MOVE_TO", 
          ...defaultLocation, 
  ...
  }, [address, zoomLevel, showDestination, isWebViewReady, lat, lng]);
  ...
  const html = `
    ...
    <body>
      ...
      <script>
        ...
        function moveTo(lat, lng, destination, zoom, showDestination) {
          try {
            console.log('📍 Moving map to:', lat, lng, 'zoom:', zoom, 'showDestination:', showDestination);
            
            // Remove existing destination marker if not showing
            if (!showDestination && destinationMarker) {
              map.removeLayer(destinationMarker);
              destinationMarker = null;
            }
            
            // Add destination marker if needed
            if (showDestination && destination && destination.lat && destination.lng) {
              if (destinationMarker) map.removeLayer(destinationMarker);
              ...
              console.log('📍 Destination marker added at:', destination.lat, destination.lng);
            }
        ...
        function handleMessage(data) {
          try {
            ...
            if (data.type === "MOVE_TO") {
              moveTo(data.lat, data.lng, data.destination, data.zoom, data.showDestination);
            } else if (data.type === "UPDATE_TANKER_LOCATION") {
              updateTankerLocation(data.lat, data.lng, data.path, data.destination, true);
            } else if (data.type === "TRACKING_STATUS") {
              handleTrackingStatus(data.isLiveTracking, data.tankerLocation);
